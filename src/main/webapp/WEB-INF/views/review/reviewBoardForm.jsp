...
<head>
    ...
    <%-- CSS 파일들을 <head> 태그 안에 명시해야 합니다. --%>
    <link rel="stylesheet" href="<c:url value='/resources/css/layout.css'/>">
    <link rel="stylesheet" href="<c:url value='/resources/css/layout.css'/>">
    <link rel="stylesheet" href="<c:url value='/resources/css/layout.css'/>"> <%-- 푸터 CSS도 있다면 추가 --%>
    ...
    <div class="container">
        ...
        <div style="display: flex; justify-content: flex-end; align-items: center; padding: 10px 0;">
            <span style="font-size: 20px; font-weight: bold; margin-right: 10px;">133,000원~</span>
            <a href="#" style="color: #007bff; text-decoration: underline;">객실보기</a>
        </div>
        --%>
        ...
        <div class="review-list">
            ...
            <c:forEach var="review" items="${review}">
                <div class="review-card" data-review-id="${review.id}"> <%-- AJAX를 위한 review ID 추가 --%>
                    <div class="review-user-info">
                        <div class="user-avatar">
                            <c:if test="${not empty review.userName}">${fn:substring(review.userName, 0, 1)}</c:if>
                            <c:if test="${empty review.userName}">익명</c:if>
                        </div>
                        <div>
                            <div class="user-name">
                                <c:choose>
                                    <c:when test="${not empty review.userName}">${review.userName}</c:when>
                                    <c:otherwise>베스트리뷰</c:otherwise>
                                </c:choose>
                            </div>
                            <div class="user-level">리뷰 ${review.reviewCount} · 사진 ${review.photoCount} · 좋아요 ${review.likeCnt}</div>
                        </div>
                    </div>
                    ...
                    <div class="review-images" id="review-images-${review.id}">
                            <%-- 이미지는 AJAX로 로드할 것이므로, 초기에는 비워둡니다. --%>
                            <%-- 또는 로딩 스피너 등을 여기에 배치할 수 있습니다. --%>
                    ...
                    <div class="review-comment-container">
                        ...
                        <c:if test="${fn:length(commentText) > maxLength}">
                            <p class="review-comment truncated-text">
                                ...
                                <span class="read-more-btn" data-target="full-comment-${review.id}">더보기</span>
                            </p>
                            <p class="review-comment full-text hidden" id="full-comment-${review.id}">
                                    ${commentText}
                                <span class="read-more-btn" data-target="truncated-comment-${review.id}">접기</span>
                            </p>
                        </c:if>
                        <c:if test="${fn:length(commentText) <= maxLength}">
                    ...
                    <c:if test="${not empty review.replyComment}">
                        <div class="admin-reply">
                            <div class="reply-header">제휴점 답변 <fmt:formatDate value="${review.repliedAt}" pattern="yy.MM.dd"/></div>
            ...
            <%-- 페이지네이션 영역 (AJAX로 동적 로딩 시에도 필요할 수 있음) --%>
 ...
 <c:if test="${totalCnt!=null && totalCnt!=0}">
    ...
    <script src="<c:url value='/resources/js/review.js'/>"></script> <%-- 리뷰 페이지 전용 JS --%>
