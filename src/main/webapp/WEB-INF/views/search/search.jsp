                    ...
                    <div class="guest-count-wrap">
                        <div class="guest-btn-wrap" tabindex="0" role="button">
                            <div class="guest-display">
                                ...
                                <span class="guest-count-text">${searchDto.guests != null ? searchDto.guests : 2}</span>
    ...
    <section id="search-page">
        ...
        <div id="cards-container" class="cards-container">
            ...
            <c:forEach var="acc" items="${accList}">
                <article id="card-${acc.accId}" class="card" onclick="location.href='/acc/${region}/${acc.accId}'"
                         style="cursor:pointer;">
...
<script>
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // 인원수 조절
    let count = parseInt(document.querySelector('.guest-count-text').textContent) || 2;

    function updateGuestCount(newCount) {
      count = Math.max(1, newCount);
      document.querySelector('.guest-count-text').textContent = count;
      document.getElementById('guests-count-input').value = count;
    }

    document.querySelector('.guest-btn.plus').addEventListener('click', () => {
      updateGuestCount(count + 1);
    });

    document.querySelector('.guest-btn.minus').addEventListener('click', () => {
      updateGuestCount(count - 1);
    });

    // 검색 버튼 클릭 시 hidden input 채우고 submit
    document.querySelector('.submit-btn').addEventListener('click', function () {
      const dateRange = document.getElementById('date-range').value;

      if (dateRange.includes('~')) {
        const [checkIn, checkOut] = dateRange.split(' ~ ');
        document.getElementById('checkInInput').value = checkIn.trim();
        document.getElementById('checkOutInput').value = checkOut.trim();
      }
